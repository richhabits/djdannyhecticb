...
async function run() {
    ...
    const [lanes] = await connection.execute('SELECT id, city, genre FROM city_lanes');
    ...
    for (const item of itmes) {
        await connection.execute(
            'INSERT INTO intel_items (laneId, category, title, content, city, genre, sourceType, sourceName, sourceUrl, sourceId, confidence, tags, publishedAt, metadata) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE content = VALUES(content)',
