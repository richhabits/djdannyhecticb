...
const BASE_URL = process.env.BASE_URL || "https://djdannyhecticb.com";
...
async function generateRSSFeed(): Promise<string> {
  ...
  if (db) {
    ...
    <category>Event</category>
      ...
      if (process.env.NODE_ENV === "development") {
        console.warn("[SEO] Error generating RSS items:", error);
      }
    }
  }
...
export function registerSEORoutes(app: Express) {
  ...
  app.get("/feed.xml", async (req: Request, res: Response) => {
    try {
      ...
      if (process.env.NODE_ENV === "development") {
        console.error("[SEO] Error generating RSS feed:", error);
      }
